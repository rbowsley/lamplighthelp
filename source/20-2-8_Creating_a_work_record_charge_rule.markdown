...
With a {{work}} record charge you can specify amounts to charge depending on the {{work}} and {{subwork}} areas chosen in the record, plus {{staff}} attending and {{location}} used. The different aspects of calculations for these rules are cumulative,  so the total charge will be {{workarea}} charges + {{subworkarea}} charges + {{staff}} charges + location charges. If a {{work}} record has a {{workarea}} and two {{subworkarea}}s, and each of these has a charge against them in the rule, the total charge will be the sum of these three (plus {{staff}} and locations). 
