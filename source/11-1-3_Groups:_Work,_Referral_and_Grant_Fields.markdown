# 11.1.3 {{Group}}s: {{Work}}, {{Referral}} and {{Grant}} Fields
...
- Next go to the 'Number of records in profile **between** date 1 and 2' (the middle line from the next set of three). You are looking for people with at least one record between those dates, so choose 'Greater than or equal to' on the drop-down, then type '1' in the text box after it.
...
- You want to find records between those dates, so next go to the 'Number of records in profile **between** date 1 and 2' (the middle box from the next set of three). You are looking for people with at least one record in their profile between those dates, so choose 'Greater than or equal to' on the drop-down, then type '1' in the text box after it.
...
- Finally, 'Initial Assessment' is a {{subworkarea}}, so first choose 'Support'under {{workarea}}, then 'Initial Assessment'.
...
- For Date 2 (your 'to' date) choose 'Fixed date' from the drop-down menu, then select 'Today'. That will filter up to the current day, whenever you run the {{group}}.
- You want to find {{people}} who have a record between those dates, so go to the 'Number of records in profile **between** date 1 and 2' (the middle line from the next set of three), choose 'Greater than or equal to' on the drop-down, then type '1' in the text box after it.
- To narrow it down to people who did not attend any {{work}} sessions before that date, go next to the line above 'Number of records in profile prior to (but not including) date 1'. From the drop-down box, choose 'Equal to', and in the next box enter '0'. This will now only show people who did not have any {{work}} records before your start date.
...
- You can select {{workarea}}s and {{subworkarea}}s to narrow down your search further. If you select multiple {{workareas}} or {{subworkareas}}, then {{Lamplight}} will include profiles of {{people}} who have been involved in any of them - they do not need to have a record for each of the areas chosen.
...
- In the drop-down box for 'Role', select '{{User}}'. This will exclude other attendees, such as {{staff}} or {{volunteer}}s.
- Finally, select the {{workarea}} 'Gardening' so that your results do not include all attendees of other {{workarea}}s.
